...
function loadAudits(): SavedAudit[] {
  try { const r = localStorage.getItem('ciq_v5'); if (!r) return []; return (JSON.parse(r) as SavedAudit[]).filter(a => Date.now() - a.ts < TTL) } catch { return [] }
}
function saveAudit(e: SavedAudit) {
  const l = loadAudits().filter(a => a.url !== e.url); l.unshift(e)
...
const ICON_PATHS: Record<string, string> = {
  chart: 'M22 12h-4l-3 9L9 3l-3 9H2', shield: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z',
    ...
    <div className={`${styles.issueItem} ${open ? styles.issueOpen : ''}`}>
      ...
      {open && (
        <div className={styles.issueBody}>
          ...
          <div className={styles.fixBox}><div className={styles.fixLbl}>Expert Fix</div><div className={styles.fixBody}>{issue.fix}</div></div>
        </div>
      )}
    </div>
  )
      ...
      {open && (
        <div className={styles.sectionBody}>
          ...
          {(section.issues?.length ?? 0) > 0 && (
            <div className={styles.sectionIssues}>{section.issues.map((iss, i) => <IssueRow key={i} issue={iss} />)}</div>
          )}
...
export default function Page() {
  ...
  const animateSteps = useCallback((onDone: () => void) => {
    const progs = [20, 45, 70, 90]; let i = 0
    const tick = () => {
      setActiveStep(i); setProgress(progs[i])
      if (i > 0) setStepsDone(prev => prev.map((v, idx) => idx < i ? true : v))
      i++; if (i < 4) setTimeout(tick, 1800 + Math.random() * 600); else onDone()
    }; tick()
  }, [])
  ...
  const runAudit = async () => {
    ...
    setError(''); const disp = normalized.replace(/^https?:\/\//, '').replace(/\/$/, '')
    setDisplayUrl(disp); setScreen('analyzing'); setProgress(0); setActiveStep(-1); setStepsDone([false,false,false,false])

    let progDone = false, apiDone = false, apiResult: AuditResult | null = null, apiError = ''

    const tryFinalize = () => {
      if (!progDone || !apiDone) return
      setProgress(100)
      if (apiError || !apiResult) { setScreen('home'); setError(apiError || 'Analysis failed. Please try again.'); return }
      ...
      setSavedAudits(loadAudits()); setResult(apiResult)
    ...
    try {
      ...
      const fetchRes = await fetch('/api/fetch-page', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: normalized }) })
      let pageContent = `Website: ${normalized}. Analyze based on domain knowledge.`
      if (fetchRes.ok) {
        const fd = await fetchRes.json() as { content?: string }
        if (fd.content && fd.content.length > 100) pageContent = fd.content
      }
      // Run audit with real content
      const auditRes = await fetch('/api/audit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: normalized, pageContent, fetchMethod: 'live' }) })
      if (!auditRes.ok) { const e = await auditRes.json() as { error?: string }; throw new Error(e.error || 'API error') }
      const ad = await auditRes.json() as { result?: AuditResult; error?: string }
      if (ad.error) throw new Error(ad.error)
      if (!ad.result) throw new Error('No result returned')
      apiResult = ad.result
    } catch (e: unknown) { apiError = e instanceof Error ? e.message : 'Something went wrong' }

    apiDone = true; tryFinalize()
  }

  const doCopy = (text: string, id: string) => {
    navigator.clipboard.writeText(text).catch(() => {}); setCopiedId(id); setTimeout(() => setCopiedId(null), 1800)
  }

  const doCustomRewrite = async () => {
    if (!customInput.trim()) return; setCustomOutput('Rewriting…')
    try {
      const res = await fetch('/api/audit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: displayUrl, pageContent: 'CUSTOM_REWRITE:\n' + customInput, fetchMethod: 'custom' }) })
  ...
  const loadSaved = (entry: SavedAudit) => {
    setResult(entry.data); setDisplayUrl(entry.display)
    setAuditTime('Saved ' + new Date(entry.auditTime).toLocaleDateString([], { month: 'short', day: 'numeric' }))
    setScreen('results'); setTab('sections'); setShowHistory(false)
  }

  // Build copy rewriter items from sections (only sections with copy_rewrite)
        ...
        <div className={styles.homeWrap}>
          <div className={styles.homeInner}>
            ...
            <div className={styles.criteriaRow}>
              {['Copy clarity','CTA strength','Trust signals','UX friction','Mobile UX','Conversion funnel'].map(l => <span key={l} className={styles.critBadge}>{l}</span>)}
            </div>
            <div className={styles.inputShell}>
              <input className={styles.urlInput} type="text" placeholder="yourwebsite.com or https://yourwebsite.com" value={urlVal} onChange={e => setUrlVal(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') runAudit() }} autoComplete="off" autoCapitalize="none" autoCorrect="off" spellCheck={false} />
            ...
            <div className={styles.prevRow}>
              {PREVIEWS.map((c, i) => (
                <div key={i} className={styles.prevCard} style={{ animationDelay: `${i * 1.7}s` }}>
                  <div className={styles.prevIcon} style={{ background: c.ibg }}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={c.ic} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d={c.icon} /></svg></div>
        ...
        <div className={styles.results}>
            ...
            <div className={styles.scoreStripInner}>
              <div className={styles.scoreGrid}>
                {SCORE_KEYS.map(([k, l], i) => {
                  const v = Math.round(result.scores[k] as number); const c = scoreColor(v)
                  return (
          ...
          <div className={styles.tabRow}>
            <div className={styles.tabRowInner}>
              {([['sections','Sections',result.sections?.length],['copy','Copy Rewriter',sectionCopyItems.length || null],['recs','Recommendations',null],['layout','Layout Blueprint',null]] as [string,string,number|null][]).map(([id,label,count]) => (
          ...
          {tab === 'sections' && (
            <div className={styles.panel}>
              <div className={styles.infoBanner}>
                <div className={styles.infoIcon}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--blue)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z" /><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z" /></svg></div>
                <div>
          ...
          {/* COPY REWRITER TAB — pulls from sections that need enhancement */}
          {tab === 'copy' && (
            <div className={styles.panel}>
              {sectionCopyItems.length > 0 ? (
                <>
                  <div className={styles.infoBanner}>
                    <div className={styles.infoIcon}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--blue)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z" /></svg></div>
                    <div>
                      <div className={styles.infoTitle}>Section copy rewrites</div>
                      <div className={styles.infoBody}>Each rewrite below is pulled from a section that needs improvement — with the exact original copy quoted from your live page and an AI-powered conversion-focused alternative.</div>
                    </div>
                  </div>
                    ...
                    return (
                      <div key={i} className={styles.copyCard}>
                        ...
                        <div className={styles.copySides}>
                          <div className={styles.copyCol}><div className={`${styles.copyTag} ${styles.ctBefore}`}>From your site</div><div className={styles.copyVal}>{cr.original}</div></div>
                          <div className={styles.copyCol}><div className={`${styles.copyTag} ${styles.ctAfter}`}>AI rewritten</div><div className={`${styles.copyVal} ${styles.copyImp}`}>{cr.improved}</div></div>
                        </div>
                ...
                <div className={styles.infoBanner}>
                  <div className={styles.infoIcon}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--blue)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg></div>
                  <div><div className={styles.infoTitle}>No section rewrites generated</div><div className={styles.infoBody}>Run the audit again to generate section-specific copy rewrites, or use the custom rewriter below.</div></div>
                </div>
              )}
              <div className={styles.customBlock}>
                ...
                <textarea className={styles.customTa} placeholder="Paste a headline, CTA, paragraph, or any copy from your site…" value={customInput} onChange={e => setCustomInput(e.target.value)} />
        ...
        <div className={styles.drawerOverlay} onClick={() => setShowHistory(false)}>
          <div className={styles.drawer} onClick={e => e.stopPropagation()}>
            ...
            {savedAudits.length === 0
              ? <div className={styles.histEmpty}><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.3 }}><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg><p>No saved audits yet.</p></div>
              : savedAudits.map((a, i) => {
                const c = a.overall >= 70 ? 'var(--green)' : a.overall >= 50 ? 'var(--amber)' : 'var(--red)'
                return (
                  <div key={i} className={styles.histItem} onClick={() => loadSaved(a)}>
                    <div className={styles.histIco}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--text2)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z" /></svg></div>
